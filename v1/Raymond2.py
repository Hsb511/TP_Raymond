...
def give_privilege(current_holder):
    global holder
   
    holder = current_holder
    print ("trying give privilege to ", request_Q, current_holder, holder, using)
	...
	asked = False
	print("done")
    else: print("condition not accepted")
...
def get_request(sender_id):
    ...
    if sender_id == holder:
	channel.queue_declare(myid+"I")
 	channel.basic_publish(exchange='',
                              routing_key=myid+"I",
                              body = "[x] " + myid + " was already the holder") 
    # transmission de request d'un noeud precedent au suivant
    elif holder != myid and request_Q != [] and not asked:
        channel.basic_publish(exchange='',
...
def callback(ch, method, properties, body):
    global crashed
    #On regarde il y a combien de temps le noeud a ete lance
    #Si le message etait en attente et qu'on a pas deja dit qu'on avait crash
    if time.time() - init_time < 1 and not crashed :
	channel.basic_publish(exchange='', routing_key = myid+"I", body="[x] "+ myid+" crashed")
	crashed = True
    message = body.split(',')
    print(message)
    sender_id = message[0]
